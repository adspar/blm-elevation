prep_study3 <- function() {
  ...
  #source("BLM-protest_study3_data-prep_00-make-raw-data-file.R")
  
  
  # ALL USERS SECTION STARTING FROM SHAREABLE RAW DATA ----
  
  # Step 1: Participant filters
  source("BLM-protest_study3_data-prep_01-participant-filters.R", local = T)
  
  # Step 2: Define our scales
  
  #### PREPARED UP THROUGH HERE SO FAR
  
  source("BLM-protest_study3_data-prep_02-define-scales.R", local = T)

  # Step 3: Reformat scale data
  source("BLM-protest_study3_data-prep_03-reformat-scale-data.R", local = T)

  # Step 4: Calculate scales
  source("BLM-protest_study3_data-prep_04-compute-scales.R", local = T)
